...
- [트러블슈팅](#트러블슈팅)
- [더 알아보기 (공식 Elastic 문서)](#더-알아보기-공식-elastic-문서)
...
- **[Elastic Agent Builder](https://www.elastic.co/elasticsearch/agent-builder)** (GA) — Elasticsearch
  데이터 위에서 동작하는 **AI Agent**를 만드는 기능. 자연어 질문을 받으면 Agent가 **스스로 추론
  (reasoning)** 하고 **도구(built-in 또는 custom, 예: ES|QL 쿼리)** 를 호출해, 답이 *데이터에 근거
  (grounded)* 하도록 만듭니다. "지난 1시간 5xx 에러 추이 분석해줘" 같은 요청을 처리합니다. Kibana 채팅
  또는 API로 접근하며, **MCP / A2A** 서버로 외부 클라이언트(Claude Desktop, Cursor, LangChain 등)에도
  노출할 수 있습니다.
- **[Elastic Workflows](https://www.elastic.co/elasticsearch/workflows)** (9.4 GA) — Elastic 플랫폼에
  내장된 **네이티브 자동화 엔진**. **YAML**로 자동화를 선언(triggers → steps → actions)하면 *데이터가
  있는 그 자리에서* 실행됩니다 — 외부 자동화 도구나 middleware가 필요 없습니다. 버전 관리 가능한
  playbook이라 보면 됩니다. Agent Builder와 Workflows는 **양방향으로 결합**합니다: agent가 workflow를
  트리거하고, workflow가 agent를 호출(`ai.agent` step)합니다.
...
| **상태 저장** | 인메모리 `THREAD_CONV` dict | **영속 저장소 — 예: 작은 Elasticsearch 인덱스(기존 연결 재사용) 또는 Redis** | 봇 재시작/다중 인스턴스에서도 **Slack thread_ts ↔ conversation_id 매핑**이 유지되어야 멀티턴이 끊기지 않습니다. (대화 *내용* 자체는 이미 Elasticsearch에 영속화됩니다 — 아래 노트 참고.) |
...
| **비용/쿼터** | 없음 | **토큰 사용량 추적 + 앱 레벨 쿼터** | Agent Builder는 대화별 **토큰 사용량**(consumption API)을 노출합니다 — 이를 모니터링하고 사용자/채널별 rate limit을 앱에서 강제합니다. Workflows 과금: Elastic Cloud Serverless는 실행 단위 과금(2026-05-01부터), Hosted/self-managed는 프로모션 기간(아직 미과금). |
...
> **상태 저장 관련 — Elasticsearch가 이미 하고 있지 않나요?** 부분적으로 맞습니다. 상태는 두 종류입니다.
> 1. **대화 내용(chat history)** — 이미 Agent Builder가 Elasticsearch에 영속화하며,
>    `GET /api/agent_builder/conversations/{id}`로 되읽습니다. 내구성은 해결돼 있고, 봇이 따로 저장할 필요가 없습니다.
> 2. **Slack `thread_ts` ↔ `conversation_id` 매핑** — 봇이 인메모리로 들고 있는 *유일한* 상태입니다.
>    Elastic은 이 연결 정보(glue)를 모릅니다(어느 conversation이 어느 Slack 스레드에 묶였는지 알지 못함).
>    봇이 재시작되면 이 매핑이 사라져, 기존 스레드의 후속 질문이 어떤 conversation으로 이어야 할지 못 찾습니다
>    — 대화 자체는 Elastic에 멀쩡히 있는데도요.
>
> 봇이 이미 Elastic에 붙어 있으니, 이 매핑의 가장 자연스러운 자리는 **작은 Elasticsearch 인덱스**
> (예: `slack-thread-map`, 문서 `{thread_ts, conversation_id}`)입니다 — 추가 인프라가 없습니다.
> Redis는 sub-ms 조회나 TTL 자동 만료가 필요할 때의 대안일 뿐입니다. 또는 **외부 저장소 없이** Slack
> 메시지 metadata에 `conversation_id`를 실어 스레드의 봇 메시지에서 되읽는 방법도 있습니다.

### 전환 체크리스트
- [ ] Socket Mode → HTTP 전환, 3초 ACK 후 작업을 **비동기 큐/워커**로 이관
- [ ] `thread_ts → conversation_id` 매핑 영속화 (기존 연결을 재사용하는 **작은 Elasticsearch 인덱스** 권장, TTL 있는 Redis는 대안)
...
**Agent Builder는 GA(9.3부터), Workflows도 GA(9.4, 기본 활성화)** 입니다. 다만 이 데모는 경로/응답
형태가 마이너 버전에 따라 달라질 수 있는 API 몇 개를 사용하고(예: async converse 스트림, 실행 로그),
일부는 아직 공식 API 레퍼런스에 다 올라와 있지 않습니다. 그러니 본인 스택에서 아래를 확인하세요(Dev
Tools에서 `GET kbn:/api/...`로 점검 가능). 클라이언트는 응답을 방어적으로 파싱합니다.
...
## 더 알아보기 (공식 Elastic 문서)

- **Elastic Agent Builder** — 개요 & 시작하기: <https://www.elastic.co/docs/explore-analyze/ai-features/elastic-agent-builder>
- **Elastic Workflows** — 개요: <https://www.elastic.co/docs/explore-analyze/workflows>
- **Elastic 9.4 신규 기능**(Workflows GA, Agent Builder 업데이트): <https://www.elastic.co/blog/whats-new-elastic-9-4-0>
- 제품 페이지: [Agent Builder](https://www.elastic.co/elasticsearch/agent-builder) · [Workflows](https://www.elastic.co/elasticsearch/workflows)
- 배경 글: [Agent Builder와 context engineering](https://www.elastic.co/search-labs/blog/elastic-ai-agent-builder-context-engineering-introduction) · [Building automation with Workflows](https://www.elastic.co/search-labs/blog/elastic-workflows-automation)

---

## Disclaimer

Elastic Solutions Architect가 **Elastic Agent Builder**와 **Elastic Workflows**를 ChatOps 맥락에서
보여주기 위해 만든 데모/POV 예제입니다. 두 기능 모두 GA(Agent Builder 9.3부터, Workflows 9.4)이지만,
이 봇은 버전 의존적이거나 아직 공식 레퍼런스에 없는 API를 일부 사용하므로 본인 스택에서는 경로/응답이
다를 수 있습니다 — 운영 적용 전 [확인 표](#스택-버전에-따라-확인할-것)와
[전환 가이드](#pov--production-전환-가이드)를 검토하세요. 이 저장소는 커뮤니티 샘플이며 공식 Elastic
제품이 아닙니다.
